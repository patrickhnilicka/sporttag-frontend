...
const riegenzuteilungEdit: Ref<Riegenzuteilung> = ref({studentId: -1,
    riegeId: undefined,
    name: '',
    sportklasse: '',
    riegeNr: undefined} as Riegenzuteilung)
...
<template>
                ...
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        <tr v-for="s in filteredZuteilungen" v-bind:key="s.studentId">
                            ...
                            <td>{{ s.sportklasse }}</td>
                            <td>{{ s.riegeNr }}</td>
                            <td><button @click="editRiegenzuteilung(s)" type="button" class="btn btn-link"
                                    data-bs-toggle="modal" data-bs-target="#riegenzuteilungEditModal"><i
                                        class="bi bi-pencil-square" style="color: green"></i></button>
                            </td>
                        </tr>
                    </tbody>
                </table>
    ...
    <RiegeZuteilungEdit :riegenzuteilung="riegenzuteilungEdit" :riegen="riegen" @save-riegenzuteilung="saveRiegenzuteilung" />
</template>
